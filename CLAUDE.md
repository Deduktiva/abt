...
### Deployment
- Production runs Apache 2.4 on `*:443` as a TLS-terminating reverse proxy in front of Puma. Puma listens on a Unix socket at `/srv/abt/tmp/sockets/puma.sock`; Apache proxies dynamic requests there and serves precompiled assets (`/assets`, `/packs`, root favicons) directly from `/srv/abt/public`. Both Puma and the Solid Queue jobs worker run as systemd **user** units under the `abt` user. Unit files live in `deploy/systemd/`; the Apache include and vhost template live in `deploy/apache/`. `bin/production-update` installs/refreshes the unit symlinks on every deploy.
