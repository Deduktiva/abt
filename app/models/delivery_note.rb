class DeliveryNote < ApplicationRecord
  ...
  def publish!
    with_lock do
      ...
      # Re-date on every publish, deliberately unlike for invoices. A delivery
      # note's date is the day it was booked, so unpublish-then-republish
      # re-stamps it to the new booking day. The document_number is gap-free
      # and stays put via `||=`, so a republished note can carry a newer date
      # than a later number — that's accepted.
